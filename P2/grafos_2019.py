...
def graph_2_multigraph(d_g):
    ...
    :param d_g: lista de adyacencia del grafo a convertir
    :return d_mg: lista de adyacencia del multigrafo con las mismas ramas que d_g
    """
    d_mg = {}
...
fl_diag=True):
    ...
    :param n_nodes: numero de nodos del grafo
    :param prob: probabilidad de que dos nodos tengan ramas entre si
    :param num_max_multiple_edges: numero maximo de ramas entre dos nodos
    :param max_weight: maximo peso de una rama
    :param decimals: numero de decimales del peso
    :param fl_unweighted: si es True, el grafo se genera sin pesos (con peso 1 si existe rama)
    :param fl_diag: True si permite auto ramas y False si no
    :return d_mg: lista de adyacencia del multigrafo creado
    """
    d_mg = {}
...
def o_a_tables(u, d_g, p, s, o, a, c):
    ...
    :param u: nodo en el que comenzamos
    :param d_g: lista de adyacencia del grafo a estudiar
    :param p: diccionario de padres
    :param s: diccionario de nodos visitados
    :param o, a: valores de orden y ascensos en cada nodo
    :param c: orden actual
    :return c: orden actual
    """
...
def p_o_a_driver(d_g, u=0):
    ...
    :param u: nodo en el que comenzamos
    :param d_g: lista de adyacencia del grafo a estudiar
    :return p, o, a: padres, orden y ascensos calculados
    """
    o = {}
    a = {}
    s = {}
...
def hijos_bp(u,p):
    ...
    :param u: nodo del que queremos obtener los hijos
    :param p: diccionario de padres
    :return hijos: lista con los hijos de u
    """
    hijos = []
    for nodo in p.keys():
        if(p[nodo] == u):
...
def check_pda(p, o, a):
    ...
    :param p: diccionario de padres
    :param o: diccionario de orden de cada nodo
    :param a: diccionario de ascenso de cada nodo
    :return pas: lista con los nodos que son puntos de articulacion
    """
    pas = []
    for nodo in p.keys():
...
def init_cd(d_g):
    ...
    :param d_g: lista de adyacencia del grafo del que queremos obtener el conjunto disjunto inicial
    :return d_cd: conjunto disjunto inicializado a -1
    """
    d_cd = {}
    for u in d_g.keys():
        d_cd[u] = -1
...
def union(rep_1, rep_2, d_cd):
    ...
    :param rep_1: representante 1
    :param rep_2: representante 2
    :param d_cd: conjunto disjunto donde queremos unir los dos representantes
    :return d_cd: conjunto disjunto nuevo
    """
...
def find(ind, d_cd, fl_cc):
    ...
    :param ind: nodo a buscar
    :param d_cd: conjunto disjunto donde queremos buscar
    :param fl_cc: True si queremos aplicar compresion de caminos, False si no
    :return rep: representante del nodo buscado
    """
    rep = ind
    while (d_cd[rep] >= 0):
        rep = d_cd[rep]
...
def insert_pq(d_g, q):
    ...
    :param d_g: grafo del que obtener las ramas
    :param q: cola de prioridad donde insertaremos las ramas
    """
    for u in d_g.keys():
        for v in d_g[u].keys():
            if u < v:
...
def kruskal(d_g, fl_cc=True):
    ...
    :param d_g: grafo donde aplicar el algoritmo
    :param fl_cc: True si queremos aplicar compresion de caminos en la busqueda, False si no
    :return aam: arbol abarcador minimo del grafo obtenido, None si el grafo no era conexo
    """
    p = init_cd(d_g)
    cola = qe.PriorityQueue()
...
def time_pda(n_graphs, n_nodes_ini, n_nodes_fin, step, prob):
    ...
    :param n_graphs: numnero de grafos a crear de cada tipo
    :param n_nodes_ini: numero de nodos inicial
    :param n_nodes_fin: numero de nodos final
    :param step: step de crecimiento del numero de nodos
    :param prob: probabilidad de que existan ramas entre nodos de los grafos creados
    :return times: diccionario con claves el numero de nodos y valores el tiempo promedio de ejecucion de la funcion
    """
    grafos = {}
    times = {}
    for n_nodos in range(n_nodes_ini,n_nodes_fin+1,step):
...
def time_kruskal(n_graphs, n_nodes_ini, n_nodes_fin, step, prob, fl_cc):
    ...
    :param step: step de crecimiento del numero de nodos
    :param prob: probabilidad de que existan ramas entre nodos de los grafos creados
    :param fl_cc: indica si queremos aplicar compresion de caminos o no
    :return times: diccionario con claves el numero de nodos y valores el tiempo promedio de ejecucion de la funcion
    """
    grafos = {}
    times = {}
...
def kruskal_2(d_g, fl_cc=True):
    """
    Funcion que aplica el algoritmo de Kruskal a un grafo y devuelve el arbol abarcador minimo obtenido y el tiempo
    que tarda en crearlo (solo el tiempo de creacion del mismo)

    :param d_g: grafo donde aplicar el algoritmo
    :param fl_cc: True si queremos aplicar compresion de caminos en la busqueda, False si no
    :return aam: arbol abarcador minimo del grafo obtenido, None si el grafo no era conexo
    :return time: tiempo que ha tardado en crear el AAM, 0 si el grafo no es conexo
    """
    p = init_cd(d_g)
    cola = qe.PriorityQueue()
    colaAux = qe.PriorityQueue()
...
def time_kruskal_2(n_graphs, n_nodes_ini, n_nodes_fin, step, prob, fl_cc):
    ...
    :param n_graphs: numero de grafos a crear de cada tipo
    :param n_nodes_ini: numero de nodos inicial
    :param n_nodes_fin: numero de nodos final
    :param step: step de crecimiento del numero de nodos
    :param prob: probabilidad de que existan ramas entre nodos de los grafos creados
